...
//=== 共通ヘルパー関数 ===//
static __device__ __forceinline__ uint64_t rotr64(uint64_t x, uint32_t n) {
    ...
    if (n < 32u) {
        ...
        uint32_t new_lo = __funnelshift_r(lo, hi, n32);
        uint32_t new_hi = __funnelshift_r(hi, lo, n32);
        return ((uint64_t)new_hi << 32) | new_lo;
    } else {
        uint32_t n2 = (uint32_t)(n - 32u);
        uint32_t new_lo = __funnelshift_r(hi, lo, n2);
        uint32_t new_hi = __funnelshift_r(lo, hi, n2);
        return ((uint64_t)new_hi << 32) | new_lo;
    }
/*
    return (x >> n) | (x << (64 - n));
...
static __device__ __forceinline__ uint64_t fBlaMka(uint64_t x, uint64_t y) {
...
static __device__ __forceinline__ void blake2b_G(uint64_t& a, uint64_t& b, uint64_t& c, uint64_t& d, uint64_t m1, uint64_t m2) {
    a = a + b + m1;
    d = rotr64(d ^ a, 32);
    c = c + d;
    b = rotr64(b ^ c, 24);
...
static __device__ __forceinline__ void store32(void *dst, uint32_t w) {
...
static __device__ __forceinline__ void blake2b_set_lastnode(blake2b_state *S) {
    S->f[1] = (uint64_t)-1;
}

static __device__ __forceinline__ void blake2b_set_lastblock(blake2b_state *S) {
    if (S->last_node) {
...
static __device__ __forceinline__ void blake2b_state_memset(blake2b_state* S) {
...
static __device__ __forceinline__ void xor_block(block* dst, const block* src) {
#pragma unroll
...
static __device__ __forceinline__ void c_memcpy(void* dest, const void* src, size_t n) {
...
__device__ __forceinline__ void fill_block(const block* prev_block, const block* ref_block, block* next_block, int with_xor) {

    block blockR;
    block block_tmp;

    // Vectorized copies (assumes alignment from block definition)
    constexpr int VEC_BYTES = (int)sizeof(ulonglong4); // 16 bytes
    static_assert((sizeof(blockR.v) % VEC_BYTES) == 0, "block.v must be multiple of 16 bytes");
    const int NUM_CHUNKS = (int)(sizeof(blockR.v) / VEC_BYTES);

    const ulonglong4* __restrict__ src_ref = reinterpret_cast<const ulonglong4*>(ref_block->v);
    ulonglong4* __restrict__ dstR = reinterpret_cast<ulonglong4*>(blockR.v);
#pragma unroll
    for (int i = 0; i < NUM_CHUNKS; ++i) dstR[i] = src_ref[i];

    const ulonglong4* __restrict__ src_prev = reinterpret_cast<const ulonglong4*>(prev_block->v);
#pragma unroll
    for (int i = 0; i < NUM_CHUNKS; ++i) {
        dstR[i].x ^= src_prev[i].x;
        dstR[i].y ^= src_prev[i].y;
        dstR[i].z ^= src_prev[i].z;
        dstR[i].w ^= src_prev[i].w;
    }

    ulonglong4* __restrict__ dstTmp = reinterpret_cast<ulonglong4*>(block_tmp.v);
#pragma unroll
    for (int i = 0; i < NUM_CHUNKS; ++i) dstTmp[i] = dstR[i];

    if (with_xor) {
        const ulonglong4* __restrict__ nextV_in = reinterpret_cast<const ulonglong4*>(next_block->v);
#pragma unroll
        for (int i = 0; i < NUM_CHUNKS; ++i) {
            dstTmp[i].x ^= nextV_in[i].x;
            dstTmp[i].y ^= nextV_in[i].y;
            dstTmp[i].z ^= nextV_in[i].z;
            dstTmp[i].w ^= nextV_in[i].w;
        }
    }
...
#pragma unroll
    ...
    ulonglong4* __restrict__ nextV = reinterpret_cast<ulonglong4*>(next_block->v);
#pragma unroll
    for (i = 0; i < NUM_CHUNKS; ++i) {
        nextV[i].x = dstTmp[i].x ^ dstR[i].x;
        nextV[i].y = dstTmp[i].y ^ dstR[i].y;
        nextV[i].z = dstTmp[i].z ^ dstR[i].z;
        nextV[i].w = dstTmp[i].w ^ dstR[i].w;
    }
/*
...
__device__ __forceinline__ void load_block(block *dst, const void *input) {
    unsigned i;
#pragma unroll
...
__device__ __forceinline__ void store_block(void *output, const block *src) {
    unsigned i;
#pragma unroll
    for (i = 0; i < ARGON2_QWORDS_IN_BLOCK; ++i) {
...
__device__ __forceinline__ int blake2b(void *out, size_t outlen, const void *in, size_t inlen,
...
//    argon2_instance_t instance;
    //    printf("t=%u dev_argon2: in=%p out=%p mem=%p t=%u m=%u lanes=%u\n",
//       threadIdx.x, (const void*)input, (void*)output, (void*)memory, t_cost, m_cost, lanes);
    // 1. メモリサイズの調整
...
/*    
    ...
    instance.threads = 1;
*/
//    printf("t=%u dev_argon2: computed memory_blocks=%u lane_length=%u segment_length=%u passes=%u lanes=%u\n", 
//        threadIdx.x, instance.memory_blocks, instance.lane_length, instance.segment_length, instance.passes, instance.lanes);

    // 2. Initial hash H0 (Blake2b prehash of parameters, input, salt)
    ...
    store32(&value, lanes);
    blake2b_update(&BlakeHash, (uint8_t*)&value, sizeof(uint32_t));
    store32(&value, 32);
    blake2b_update(&BlakeHash, (uint8_t*)&value, sizeof(uint32_t));
    // memory_blocks (aligned m_cost)
    ...
    blake2b_update(&BlakeHash, (uint8_t*)&value, sizeof(uint32_t));
    // input_len and input
    store32(&value, (uint32_t)input_len);
    blake2b_update(&BlakeHash, (uint8_t*)&value, sizeof(uint32_t));
    blake2b_update(&BlakeHash, (const uint8_t*)input, input_len);
    // salt_len
    const uint8_t RinSalt[11] = { 'R','i','n','C','o','i','n','S','a','l','t' };
    size_t salt_len = sizeof(RinSalt);
    store32(&value, salt_len);
    blake2b_update(&BlakeHash, (uint8_t*)&value, sizeof(uint32_t));
    // salt
    blake2b_update(&BlakeHash, (const uint8_t *)RinSalt, salt_len);
    // secret_len (0)
    store32(&value, 0);
    blake2b_update(&BlakeHash, (uint8_t*)&value, sizeof(uint32_t));
    // ad_len (0)
...
__global__ void argon2d_hash_device_kernel(
    ...
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        device_argon2d_hash(output, input, input_len, t_cost, m_cost, lanes, memory_ptr);
    }
}
