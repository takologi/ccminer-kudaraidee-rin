...
__device__ __forceinline__ u32 g_rotr(u32 value, int shift) {
    shift &= 31;
    return __funnelshift_r(value, value, (unsigned)shift);
/*
    return (value >> shift) | (value << ((32 - shift) & 31));
*/
}

static __device__ void g_g(u32 state[16], u32 a, u32 b, u32 c, u32 d, u32 mx, u32 my) {
...
static __device__ void g_compress(
    u32 *chaining_value,
    u32 *block_words,
    u64 counter,
    u32 block_len,
...
static __device__ void g_words_from_little_endian_bytes(
...
static __global__ void compute(Chunk *data, int l, int r) {
    int n = r-l;
    int tid = blockDim.x * blockIdx.x + threadIdx.x;
    if(tid >= n)
        return;

    // Only process leaf nodes in device kernel
    if(n==1) {
        data[l].g_compress_chunk();
        // printf("Compressing : %d\n", l);
    }
    // else: recursion must be handled on host side
}
